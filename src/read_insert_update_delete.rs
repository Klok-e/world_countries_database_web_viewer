...
fn cities_update(conn: OracleConnection, item: Json<City>) -> Result<Json<City>, Error> {
    update_data(&*conn, item.0.clone())?;
    Ok(item)
}
...
fn countries_update(conn: OracleConnection, item: Json<Country>) -> Result<Json<Country>, Error> {
    update_data(&*conn, item.0.clone())?;
    Ok(item)
}
...
fn districts_update(conn: OracleConnection, item: Json<District>) -> Result<Json<District>, Error> {
    update_data(&*conn, item.0.clone())?;
    Ok(item)
}
...
fn regions_update(conn: OracleConnection, item: Json<Region>) -> Result<Json<Region>, Error> {
    update_data(&*conn, item.0.clone())?;
    Ok(item)
}
